...
import static org.stingle.photos.Crypto.Crypto.sha256;
...
public class MnemonicUtils {
	 ...
	 */
	public static String generateMnemonic(Context context, byte[] initialEntropy) throws IOException {
		validateKey(initialEntropy);
		final List<String> words = getWords(context);
	 ...
	 */
	public static byte[] generateKey(Context context, String mnemonic) throws IOException {
		final BitSet bits = new BitSet();
		final int size = mnemonicToBits(context, mnemonic, bits);
		if (size == 0) {
	...
	public static List<String> getWords(Context context) throws IOException {
		if (WORD_LIST == null) {
			WORD_LIST = Collections.unmodifiableList(populateWordList(context));
		}
		return WORD_LIST;
	}
	...
	private static void validateKey(byte[] entropy) {
		if (entropy == null) {
	...
	private static int mnemonicToBits(Context context, String mnemonic, BitSet bits) throws IOException {
		int bit = 0;
		final List<String> vocabulary = getWords(context);
	...
	private static List<String> populateWordList(Context context) throws IOException {
