...
public class FingerprintManagerWrapper {
                    ...
                    LoginManager.getPasswordFromUser(context, false, new PasswordReturnListener() {
                        ...
                        public void passwordReceived(String password) {
                            try {
                                ...
                                preferences.edit().putString(StinglePhotosApplication.PASSWORD_FINGERPRINT, StinglePhotosApplication.getCrypto().byte2hex(encPassword)).commit();
                                preferences.edit().putString(StinglePhotosApplication.PASSWORD_FINGERPRINT_IV, StinglePhotosApplication.getCrypto().byte2hex(iv)).commit();
    ...
    public boolean unlock(final PasswordReceivedHandler passwordHandler, final LoginManager.UserLogedinCallback loginCallback, boolean showLogout){
        ...
        final SharedPreferences preferences = context.getSharedPreferences(StinglePhotosApplication.DEFAULT_PREFS, Context.MODE_PRIVATE);

        String storedIvHex = preferences.getString(StinglePhotosApplication.PASSWORD_FINGERPRINT_IV, "");
        byte[] storedIv = StinglePhotosApplication.getCrypto().hex2byte(storedIvHex);
        if (initCipherDecrypt(storedIv)) {
            ...
            FingerprintHandler helper = new FingerprintHandler(context, new FingerprintHandler.OnFingerprintSuccess() {
                ...
                public void onSuccess(Cipher cipher) {
                    ...
                    String encPass = preferences.getString(StinglePhotosApplication.PASSWORD_FINGERPRINT, "");

                    try {
                        byte[] decPassBytes = cipher.doFinal(StinglePhotosApplication.getCrypto().hex2byte(encPass));
