...
### Auth — `Auth/`
`KeyManagement` (generate/import/backup keys, key bundles), `LoginManager` (login/logout/lock flows, password prompts), `BiometricsManagerWrapper` (fingerprint/face unlock). For biometrics, the app generates a 256-bit key, encrypts the user's password with it (AES256/PKCS7), and stores **that key in the Android Keystore** — the password itself is never stored in plaintext, and biometric unlock effectively decrypts the password to derive the key. The **API bearer token is encrypted at rest** by `Auth/TokenStore` (AndroidKeyStore AES-256-GCM, no user-auth requirement so background sync can read it while locked; transparently migrates any legacy plaintext token). `KeyManagement.getApiToken/setApiToken/removeApiToken` delegate to it — never read the `API_TOKEN` pref directly.
...
### Local database — `Db/`
Raw SQLite (no Room). `StingleDb` + `StingleDbContract` define schema; `Db/Query/*Db.java` are per-table DAOs (`FilesDb`, `AlbumsDb`, `AlbumFilesDb`, `ContactsDb`, `GalleryTrashDb`, `ImportedIdsDb`). `Db/Objects/*` are the row models (`StingleDbFile`, `StingleDbAlbum`, `StingleContact`). The DB stores **encrypted metadata**; values are decrypted on read via `Crypto`. DAO queries must be **parameterized** (`?` + selectionArgs), never string-concatenated — server-supplied values (filenames, album ids) are untrusted.

### Untrusted-input handling (server is the attacker)
Because the server is untrusted, every value it supplies that touches the filesystem must be validated. Storage filenames are random base64url + `.sp`, so any server `file` value is checked with `FileManager.isValidStorageFilename` at the JSON boundary (`StingleDbFile(JSONObject)`) — rejecting path separators / `..` to prevent traversal writes. The user-visible filename inside a decrypted file header (attacker-controllable when a file is shared) is reduced to its base name with `FileManager.sanitizeToBaseName` before any export/`MediaStore` write. `Crypto.java` parsers bound every length field against `MAX_BUFFER_LENGTH` and guard `len >= Box.SEALBYTES` before `new byte[len - SEALBYTES]` to avoid OOM/`NegativeArraySizeException` from a malformed `.sp`/album blob. Don't log credentials or plaintext metadata (tokens, password/login hash, request params, response bodies, original filenames, contact emails) — those logs were removed in the 2026-06 security review. Decrypted content activities are `FLAG_SECURE` by default (`BLOCK_SCREENSHOTS` pref defaults true).
